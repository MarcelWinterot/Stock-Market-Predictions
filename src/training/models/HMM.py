"""
Code taken from https://github.com/lorenlugosch/pytorch_HMM/tree/master and modified to work with only pytorch
"""
...
class HMM(torch.nn.Module):
    ...
    def sample(self, T=10):
        ...
        z_t = torch.distributions.categorical.Categorical(
            state_priors).sample().item()
        z = []
        x = []
        z.append(z_t)
        for t in range(0, T):
            # sample emission
            x_t = torch.distributions.categorical.Categorical(
                emission_matrix[z_t]).sample().item()
            x.append(x_t)

            # sample transition
            z_t = torch.distributions.categorical.Categorical(
                transition_matrix[:, z_t]).sample().item()
            if t < T-1:
                z.append(z_t)
...
def log_domain_matmul(log_A, log_B, use_max=False):
    ...
    computes out_{i,j} = logsumexp_k log_A_{i,k} + log_B_{k,}

    This is needed for numerical stability
    when A and B are probability matrices.
    """
    out = torch.bmm(log_A.unsqueeze(2), log_B.unsqueeze(1))
...
class TransitionModel(torch.nn.Module):
    ...
    def forward(self, log_alpha, use_max):
        ...
        if use_max:
            out1, out2 = maxmul(log_transition_matrix,
                                log_alpha.transpose(0, 1))
